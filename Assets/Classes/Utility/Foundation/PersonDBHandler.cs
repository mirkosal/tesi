...
{
    private static PersonDBHandler _instance;
    private static readonly object _lock = new object();
    public const string NomeTabella = "Persons";

    private PersonDBHandler(string DB) : base(DB, NomeTabella)
    {
        UnityEngine.Debug.Log(DB);
    }

    public static PersonDBHandler Instance(string dbPath)
    {
        if (_instance == null)
        {
            lock (_lock)
            {
                if (_instance == null)
                {
                    _instance = new PersonDBHandler(dbPath);
                }
            }
        }
        return _instance;
    ...
    public override List<Person> Search(params object[] parameters)
    {
        ...
        using (IDbConnection dbConnection = new SqliteConnection(connectionString))
        {
            ...
            using (IDbCommand dbCmd = dbConnection.CreateCommand())
            {
                ...
                {
                    sqlQuery = $"SELECT * FROM Persons WHERE {columnName} LIKE @SearchTerm";
